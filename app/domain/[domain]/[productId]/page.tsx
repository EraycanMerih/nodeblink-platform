...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      images: [
        {
          url: `${origin}/pay/${creator.username}/${productId}/opengraph-image`,
          width: 1200,
          height: 630,
    ...
    twitter: {
      ...
      title: `${product.title} — ${price}`,
      description,
      images: [`${origin}/pay/${creator.username}/${productId}/opengraph-image`],
    },
    other: {
      'twitter:player': `${origin}/embed/pay/${creator.username}/${productId}`,
...
export default async function DomainProductPayPage({ params }: Props) {
  const { domain, productId } = await params;
  const origin = await getRequestOrigin();
